...
public class CambioController {
	...
	public ResponseEntity<CambioEntity> getCambio(@PathVariable double valor, @PathVariable String origem,
		...
		System.out.println("Requisição feita pelo usuário: " + usuario);
	...
	public CambioEntity getCambioFromBancoCentral(String origem, String destino) {
		...
		double fator;
		System.out.println("Tentativa via API Banco Central");
		CotacaoResponse cotacaoOrigem = cotacaoClient.getCotacao(origem, "10-24-2024");
		double fatorOrigem = cotacaoOrigem.getValue().get(0).getCotacaoVenda();
		if (destino.equals("BRL")) {
			fator = fatorOrigem;
		} else {
			CotacaoResponse cotacaoDestino = cotacaoClient.getCotacao(destino, "10-24-2024");
